...
contract Tracking {
    enum ShipmentStatus { PENDING, IN_TRANSMIT, DELIVERED }
    ...
    function createShipment(address _receiver, uint256 _pickupTime, uint256 _distance, uint256 _price) public payable {
        ...
        TypeShipment memory tShipment = TypeShipment(msg.sender, _receiver, _pickupTime, 0, _distance, _price, ShipmentStatus.PENDING, false);
        typeShipments.push(tShipment);
        emit ShipmentCreated(msg.sender, _receiver, _pickupTime, _distance, _price);
    }
    ...
    function startShipment(address _sender, address _receiver, uint256 _index) public {
        ...
        require(shipment.status == ShipmentStatus.PENDING, "Shipment already in transmit");
    ...
    function completeShipment(address _sender, address _receiver, uint256 _index) public {
        Shipment storage shipment = shipments[_sender][_index];
        TypeShipment storage tShipment = typeShipments[_index];

        require(shipment.receiver == _receiver, "Invalid receiver.");
        require(shipment.status == ShipmentStatus.IN_TRANSMIT, "Shipment not in transmit.");
        require(!shipment.isPaid, "Shipment already isPaid");

        shipment.status = ShipmentStatus.DELIVERED;
        tShipment.deliveryTime = block.timestamp;
